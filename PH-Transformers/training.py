...
class Trainer():
    ...
    def train2(self, train_data, val_data, pad_idx):
        for epoch in range(self.epochs):
            epoch_start_time = time.time()
            ...
            for (src, tgt) in train_data:
                ...
                tgt_input = tgt[:-1, :]
                logits = self.net(src, tgt_input, pad_idx)
            ...
            print((f"[Epoch: {epoch}][Train loss: {train_loss:.3f}][Val loss: {val_loss:.3f}]"
                   f"[Val ppl: {math.exp(val_loss):.3f}][Time: {(epoch_end_time - epoch_start_time):.3f}s]"))
            
            wandb.log({"train loss": train_loss})
            wandb.log({"val loss": val_loss})
            wandb.log({"val ppl": math.exp(val_loss)})

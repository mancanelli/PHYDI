...
if __name__ == '__main__':
    ...
    net = GetModel(str_model=model, quat_data=quat_data, n=n, num_classes=num_classes, rezero=rezero)
    ...
    if opt.scheduler == "cosine":
        ...
        scheduler = MultiStepLR(optimizer, milestones=[100, 150]) # milestones=[59, 119, 159], gamma=0.15)
    ...
    trainer = Trainer(net, optimizer, scheduler, epochs=epochs, quat_data=quat_data, n=n, 
                      ...
                      checkpoint_folder = checkpoint_folder, saveModelsPerEpoch=EpochCheckpoints,
                      get_iter_time=get_iter_time, get_inf_time=get_inf_time, optim_name=optim_name,
                      lr=lr, momentum=opt.momentum, weight_decay=opt.weight_decay, l1_reg=l1_reg)
    
    print("model: {}".format(opt.model))
    print("rezero: {}".format(opt.rezero))
    print("n: {}".format(opt.n))
